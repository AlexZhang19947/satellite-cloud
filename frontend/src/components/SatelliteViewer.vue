...
async function initializeViewer() {
  try {
    ...
    // 如果没有提供 scenarioId，获取第一个场景的 ID
    let actualScenarioId = props.scenarioId
    if (!actualScenarioId) {
      const scenariosResponse = await getScenarios()
      // 处理分页响应格式 {count, next, previous, results: [...]}
      const scenarios = scenariosResponse.results || (Array.isArray(scenariosResponse) ? scenariosResponse : [])
      if (!scenarios || scenarios.length === 0) {
        throw new Error('没有可用的场景数据，请先导入场景数据')
      }
      actualScenarioId = scenarios[0].id
      console.log(`未提供场景ID，自动使用第一个场景: ${actualScenarioId}`)
    }
    
    // 从后端获取场景和卫星数据
    const { scenario, satellites } = await getScenarioWithSatellites(actualScenarioId)
